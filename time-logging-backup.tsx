...
export default function TimeLogging() {
  ...
  const submitWeekMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your weekly timesheet has been submitted successfully.",
      });
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to submit weekly timesheet. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const unsubmitWeekMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Your weekly timesheet has been reopened for editing.",
      });
      // Invalidate the specific weekly submission query that the UI is using
      queryClient.invalidateQueries({ queryKey: ['/api/resources', selectedResourceId, 'weekly-submissions', 'week', selectedWeek] });
      // Also invalidate the general weekly submissions query for good measure
  ...
  const saveEntry = async (allocationId: number) => {
    ...
    // Set saving state
    setSavingStates(prev => ({ ...prev, [allocationId]: true }));
    setSavedStates(prev => ({ ...prev, [allocationId]: false }));

    // Sanitize hour values - convert empty strings to "0.00"
  ...
  const saveAllEntries = async () => {
    ...
    const promises = Object.keys(weekForm.entries).map(allocationId => 
      saveEntry(parseInt(allocationId))
    );
      ...
      <main className="p-6 max-w-7xl mx-auto">
        ...
        {selectedResourceId && (
          <>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-4">
                              ...
                              <Button 
                                ...
                                disabled={isSaving}
                                className="rounded-full h-10 px-4 bg-white/70 backdrop-blur-sm border-slate-200 hover:bg-white/90 transition-all duration-200 shadow-sm hover:shadow-md"
                              >
                                {isSaving ? (
                            ...
                            {weekDays.map((day, index) => {
                              ...
                              return (
                                <div key={day} className="space-y-2">
                                  ...
                                  <div className="relative">
                                    <Input
                                      ref={(el) => inputRefs.current[`${allocation.id}-${dayKeys[index]}`] = el}
                                      type="text"
                                      value={weekForm.entries[allocation.id]?.[dayKeys[index]] || ''}
                                      onChange={(e) => handleHoursChange(allocation.id, dayKeys[index], e.target.value)}
                                      onBlur={(e) => handleHoursBlur(allocation.id, dayKeys[index], e.target.value)}
                                      ...
                                      className={`time-input h-12 text-center transition-all duration-200 ${
                                        focusedField === `${allocation.id}-${dayKeys[index]}` 
                                          ? 'ring-2 ring-blue-500 border-blue-500 bg-white scale-105' 
                                          : ''
                                      } ${
                                        isCurrentDay && !isDisabled && !weeklySubmission?.isSubmitted
                                          ? 'bg-blue-50 border-blue-200'
                                          : ''
                                      } ${
                                        isDisabled || weeklySubmission?.isSubmitted
                                          ? 'bg-slate-100 text-slate-400 cursor-not-allowed'
                                          : ''
                                      }`}
                                      placeholder="0.00"
                                    />
                                    {parseFloat(weekForm.entries[allocation.id]?.[dayKeys[index]] || '0') > 0 && (
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div className="relative w-20">
                                        <Input
                                          ...
                                          onFocus={() => setFocusedField(`${allocation.id}-${dayKeys[index]}`)}
                                          disabled={isDisabled || weeklySubmission?.isSubmitted}
                                          className={`time-input h-12 text-center font-medium transition-all duration-200 ${
                                            focusedField === `${allocation.id}-${dayKeys[index]}` 
                                              ? 'ring-2 ring-blue-500 border-blue-500 bg-white scale-105' 
                                              : 'bg-white/90'
                                          } ${
            ...
            {selectedResourceId && activeAllocations.length > 0 && (
              <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-slate-200 p-4 z-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-slate-600">
                    Total: {calculateWeeklySummary().totalHours.toFixed(1)}h
                  </div>
                  <Button
